...
const ProposalForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isSpecializedProposal && (!formData.scheduledDate || !formData.scheduledStartTime || !formData.scheduledEndTime)) {
      toast.error('Scheduled date, start time, and end time are required for specialized proposals');
      return;
    }
    ...
    if (isSpecializedProposal && (formData.proposalItems || []).length === 0 && (formData.proposalServices || []).length === 0) {
      toast.error('Add at least one specialized service or line item');
      return;
    }
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Select
                label="Proposal Type *"
                value={formData.proposalType || 'recurring'}
                onChange={(value) => {
                  handleChange('proposalType', value);
                  if (value === 'specialized' || value === 'one_time') {
                ...
                options={[
                  ...
                  { value: 'specialized', label: 'One-Time Specialized Job' },
                ]}
              />
              <Select
                label="Account *"
