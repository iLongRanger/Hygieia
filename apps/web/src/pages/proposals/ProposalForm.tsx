...
const ProposalForm = () => {
  ...
  const handleSpecializedCatalogItemChange = (catalogItemId: string) => {
    ...
    setFormData((prev) => {
      ...
      const nextTitle = defaultSpecializedProposalTitle(catalogItem.name, selectedFacility?.name);
      if (shouldReplaceTitle) lastAutoGeneratedTitleRef.current = nextTitle;

      return {
        ...prev,
        title: shouldReplaceTitle ? nextTitle : prev.title,
