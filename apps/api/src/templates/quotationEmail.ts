...
export function buildQuotationEmailHtml(data: QuotationEmailData): string {
  ...
  return buildQuotationEmailHtmlWithBranding(data, branding);
}

export function buildQuotationEmailHtmlWithBranding(
        ...
        <table cellpadding="0" cellspacing="0" width="600" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
          ...
          <tr>
            <td style="padding: 30px;">
              <h2 style="color: ${branding.themePrimaryColor}; margin: 0 0 10px 0; font-size: 20px;">Quotation: ${escapeHtml(data.title)}</h2>
              <p style="color: #666; margin: 0 0 20px 0; font-size: 14px;">${escapeHtml(data.quotationNumber)}</p>
              ...
              <p style="color: #333; font-size: 14px; line-height: 1.6;">
                Please find the attached quotation for your review. You can view the full details and respond online using the button below.
              </p>
            </td>
          </tr>
...
export function buildQuotationEmailSubject(quotationNumber: string, title: string): string {
  return `Quotation ${quotationNumber}: ${title}`;
}
